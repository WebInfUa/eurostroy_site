...
if($action=='add'){
	...
	foreach($lang as $val){
		...
		$db=new db();
		$update=$db->insert('rce_pages',$data);
	}
	rce_log($_SESSION['user']['email'],'edit','Создал страницу '.$page_title);
	...
	foreach($lang as $val){
		...
		$db=new db();
		$update=$db->update('rce_pages',$data,'`pageID`=\''.$id.'\' AND `lang`=\''.$val.'\'');
	}
	rce_log($_SESSION['user']['email'],'edit','Изменил страницу '.$page_title);
	header("Location: /admin/?module=".rce_get_currentdir()."&status=".$action."_ok");
	...
	$id=$_POST['id'];
	$db=new db();
	$delete=$db->delete('rce_pages',"`pageID`='$id'");
	rce_log($_SESSION['user']['email'],'delete','Удалил страницу с ID: '.$id);
	header("Location: /admin/?module=".rce_get_currentdir()."&status=".$action."_ok");
}

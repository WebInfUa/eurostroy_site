...
if(!isset($_GET['action'])){ // NO ACTIONS
	...
	$test_result=mysql_query("
		SELECT * FROM `rce_news_cats` WHERE `lang`='ru' ORDER BY `ID` ASC
	");
  ...
  <tr class="table-info__param">
    ...
    <td class="table-info__param--title">
      ...
      <p class="txt-error">Порядок: '.$f['order'].'</p>
    </td>
  ...
  <a class="button button__access" href="'.RCE_ADMIN.'?module='.$_GET['module'].'&action=add">
    <span class="fa fa-plus"></span>&nbsp;Добавить рубрику
  </a>
  ';
	} else {$add='';}
...
<main class="wrapper-main">
 <section class="section-header">
  ...
  <section class="wrapper-main">
    <section class="section-header">
      <p class="info-headers">Список созданных рубрик</p>
        ...
        <section class="wrapper-main '.$panel_active.'" id="'.$f['lang'].'">
          <div class="data-page__panel--color">
            <div class="data-page__panel--title">
              <label for="'.$f['lang'].'_title">Название рубрики:</label>
              <input class="data-page__panel--input" id="'.$f['lang'].'_title" type="text" name="'.$f['lang'].'_title" required placeholder="Название рубрики новостей."/>
            </div>
            <div class="data-page__panel--title">
              <label for="'.$f['lang'].'_order">Порядок:</label>
              <input class="data-page__panel--input" id="'.$f['lang'].'_order" type="text" name="'.$f['lang'].'_order" required placeholder="Порядок рубрики"/>
            </div>
            <div class="data-page__panel--title">
              <label for="'.$f['lang'].'_trans">URL адресс:</label>
              <input class="data-page__panel--input" id="'.$f['lang'].'_trans" type="text" name="'.$f['lang'].'_trans" required placeholder="Введите url рубрики"/>
            </div>
          </div>
          <div class="data-page__panel">
            <label for="'.$f['lang'].'_meta_title">МЕТА заголовок:</label>
            <input class="data-page__panel--widinput" id="'.$f['lang'].'_meta_title" type="text" name="'.$f['lang'].'_meta_title" required placeholder="Мета заголовок рубрики."/>
          </div>
          <div class="data-page__panel--color">
          ...
          <div class="data-page__panel">
            ...
            <textarea class="data-page__panel--area" id="'.$f['lang'].'_meta_desc" name="'.$f['lang'].'_meta_desc" placeholder="Описание рубрики, опишите содержание рубрики. Не больше 150 символов"></textarea>
          </div>
          <div class="data-page__panel--color">
...
<main class="wrapper-main">
  ...
  <section class="wrapper-main">
    <section class="section-header">
      ...
      <div>
        <input form="rubricCreateForm" class="button button__access" type="submit" value="Добавить рубрику"/>
        <a class="button button__cancel" href="'.RCE_ADMIN.'?module='.$_GET['module'].'">Отменить</a>
      </div>
    </section>

    <form id="rubricCreateForm" action="'.RCE_ADMIN.'modules/'.$_GET['module'].'/update.php" method="post">
...
';
		...
		// General info
		$mod=$db->select('SELECT * FROM `rce_news_cats` WHERE `ID`='.$_GET['id'],'','','');
		$title='Редактировать рубрику '.$mod['title'].' - '.$module['title'];
		// Lang list
		$r=mysql_query("SELECT * FROM `rce_config_lang` WHERE `active` ORDER BY `ID` ASC");
		for($i=0;$i<mysql_num_rows($r);$i++){
			$f=mysql_fetch_array($r);
				...
				$langs.=$f['lang'].$lang_end;
				// Checking page lang
				$count=$db->count("SELECT * FROM `rce_news_cats` WHERE `pageID`='".$mod['ID']."' AND `lang`='".$f['lang']."'");
				if($count=='1'){ // Got this page with current lang
					$page=$db->select("SELECT * FROM `rce_news_cats` WHERE `pageID`='".$mod['ID']."' AND `lang`='".$f['lang']."'",'','','');
        ...
        <section class="wrapper-main '.$panel_active.'" id="'.$f['lang'].'">
          <div class="data-page__panel--color">
            <div class="data-page__panel--title">
              <label class="" for="'.$f['lang'].'_title">Название рубрики:</label>
              <input class="data-page__panel--input" id="'.$f['lang'].'_title" type="text" name="'.$f['lang'].'_title" required placeholder="Название рубрики новостей." value="'.$page['title'].'"/>
            </div>
            <div class="data-page__panel--title">
              <label for="'.$f['lang'].'_order">Порядок:</label>
              <input class="data-page__panel--input" id="'.$f['lang'].'_order" type="text" name="'.$f['lang'].'_order" required placeholder="Порядок рубрики" value="'.$page['order'].'"/>
            </div>
            <div class="data-page__panel--title">
              <label for="'.$f['lang'].'_trans">URL адресс:</label>
              <input class="data-page__panel--input" id="'.$f['lang'].'_trans" type="text" name="'.$f['lang'].'_trans" required placeholder="Введите url рубрики" value="'.$page['trans'].'"/>
            </div>
          </div>
          <div class="data-page__panel">
            <label for="'.$f['lang'].'_meta_title">МЕТА заголовок:</label>
            <input class="data-page__panel--widinput" id="'.$f['lang'].'_meta_title" type="text" name="'.$f['lang'].'_meta_title" required placeholder="Мета заголовок рубрики." value="'.$page['meta_title'].'"/>
          </div>
          <div class="data-page__panel--color">
            <label for="'.$f['lang'].'_meta_keys">МЕТА ключи:</label>
          ...
          <div class="data-page__panel">
            <label for="'.$f['lang'].'_meta_desc">МЕТА описание:</label>
            <textarea class="data-page__panel--area" id="'.$f['lang'].'_meta_desc" name="'.$f['lang'].'_meta_desc" placeholder="Описание рубрики, опишите содержание рубрики. Не больше 150 символов">'.$page['meta_desc'].'</textarea>
          </div>
          <div class="data-page__panel--color">
            <label for="'.$f['lang'].'_textArea">Содержание</label>
...
<main class="wrapper-main">
  ...
  <section class="wrapper-main">
    <section class="section-header">
      <p class="info-headers">Форма редактирования рубрики</p>
      <div>
        <input form="rubricEditForm" class="button button__access" type="submit" value="Сохранить настройки"/>
        <a class="button button__cancel" href="'.RCE_ADMIN.'?module='.$_GET['module'].'">Отменить</a>
      </div>
    </section>
    
    <form id="rubricEditForm" action="'.RCE_ADMIN.'modules/'.$_GET['module'].'/update.php" method="post">
      <input type="hidden" name="action" value="'.$_GET['action'].'" />
      <input type="hidden" name="id" value="'.$_GET['id'].'" />
      <input type="hidden" name="langs" value="'.$langs.'" />
      <section class="panel-body">
...
';
		...
		$lang=$db->select('SELECT * FROM `rce_news_cats` WHERE `ID`='.$_GET['id'],'','','');
...
<main class="wrapper-main">
  ...
  <section class="wrapper-main">
    <section class="section-header">
      <p class="info-headers">Подтверждение для удаления рубрики</p>
      <div>
        <input form="rubricDeleteForm" class="button button__edit" type="submit" value="Удалить рубрику!"/>
        <a class="button button__cancel" href="'.RCE_ADMIN.'?module='.$_GET['module'].'">Отменить</a>
      </div>
    </section>

    <form id="rubricDeleteForm" action="'.RCE_ADMIN.'modules/'.$_GET['module'].'/update.php" method="post">
      <input type="hidden" name="action" value="'.$_GET['action'].'" />
      <input type="hidden" name="id" value="'.$_GET['id'].'" />
      <section class="panel-body">
        <p class="txt-error page-header"><strong>Вы уверены, что хотите удалить рубрику:</strong><br /><br /> '.$lang['title'].'?</p>
      </section>
    </form>

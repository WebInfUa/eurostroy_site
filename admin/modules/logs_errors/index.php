...
if(!isset($_GET['action'])){ // NO ACTIONS
	...
	$test_result=mysql_query("SELECT * FROM `rce_logs_errors` ORDER BY `ID` DESC");
	...
	$r=mysql_query("SELECT * FROM `rce_logs_errors` ORDER BY `ID` DESC ".$LIMITS);
	...
	// Cases
	if($action=='clear'){
		$title='Очистить логи ошибок - '.$module['title'];
		$content.='
			<div id="page-wrapper">
			 <div class="row">
					<div class="col-lg-12">
									...
									<form action="'.RCE_ADMIN.'modules/'.$_GET['module'].'/update.php" method="post">
										...
										<table class="table no-border">
											<tbody>
												<tr>
													<td>
														Вы уверены, что хотите <b>очистить все</b> логи ошибок?
													</td>
													<td> </td>
												</tr>
												<tr>

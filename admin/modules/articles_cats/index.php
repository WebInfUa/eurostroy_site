...
if(!isset($_GET['action'])){ // NO ACTIONS
	...
	$test_result=mysql_query("
		SELECT * FROM `rce_articles_cats` WHERE `lang`='ru' ORDER BY `ID` ASC
	");
		...
		$f=mysql_fetch_array($r);
			...
			$list.='
				<tr>
					...
					<td>
						'.$f['title'].'
						<div class="infoblock">Порядок: '.$f['order'].'</div>
					</td>
	...
	if($action=='add'){
						...
						<table class="table no-border">
							<tbody>
								<tr>
									...
									<td><input name="'.$f['lang'].'_title" class="form-control" placeholder="Название рубрики новостей" /></td>
								</tr>
								...
								<tr>
									<td width="150">Порядок:</td>
									<td><input name="'.$f['lang'].'_order" class="form-control" placeholder="Порядок вывода рубрики, 0 и выше" /></td>
								</tr>
								<tr>
									<td>МЕТА заголовок</td>
									<td><input name="'.$f['lang'].'_meta_title" class="form-control" placeholder="Мета заголовок рубрики" /></td>
								</tr>
								<tr>
									<td>МЕТА ключи</td>
								...
								<tr>
									<td>МЕТА описание</td>
									<td><input name="'.$f['lang'].'_meta_desc" class="form-control" placeholder="Описание рубрики, опишите вкратце о чем страница" /></td>
								</tr>
								<tr>
									<td></td>
									<td>
										<input type="submit" value="Добавить рубрику" class="btn btn-primary btn-success" />
									</td>
								</tr>
							</tbody>
						</table>
		...
		// General info
		$mod=$db->select('SELECT * FROM `rce_articles_cats` WHERE `ID`='.$_GET['id'],'','','');
		...
		for($i=0;$i<mysql_num_rows($r);$i++){
			$f=mysql_fetch_array($r);
				...
				$langs.=$f['lang'].$lang_end;
				// Checking page lang
				$count=$db->count("SELECT * FROM `rce_articles_cats` WHERE `pageID`='".$mod['ID']."' AND `lang`='".$f['lang']."'");
				if($count=='1'){ // Got this page with current lang
					$page=$db->select("SELECT * FROM `rce_articles_cats` WHERE `pageID`='".$mod['ID']."' AND `lang`='".$f['lang']."'",'','','');
					...
					$db->insert('rce_articles_cats',$data);
					$page=$db->select("SELECT * FROM `rce_articles_cats` WHERE `pageID`='".$mod['ID']."' AND `lang`='".$f['lang']."'",'','','');
				}
				// Tab button add
				if($i=='0'){$tab_active='class="active"';}else{$tab_active='';}
		...
		$lang=$db->select('SELECT * FROM `rce_articles_cats` WHERE `ID`='.$_GET['id'],'','','');
									...
									<form action="'.RCE_ADMIN.'modules/'.$_GET['module'].'/update.php" method="post">
										...
										<table class="table no-border">
											<tbody>
												<tr>
													<td>
														Вы уверены, что хотите <b>удалить</b> рубрику статей: '.$lang['title'].'?
													</td>
													<td> </td>
												</tr>
												<tr>

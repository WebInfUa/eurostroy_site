...
if($action=='add'){
	// Add new page
	$db=new db();
	$nextpage=$db->getnext('rce_articles_cats');
	// Post data
	$langs=$_POST['langs'];
	// Fetching lang list
	$lang=explode(',',$langs);
	foreach($lang as $val){
		...
		$db=new db();
		$update=$db->insert('rce_articles_cats',$data);
	}
	rce_log($_SESSION['user']['email'],'edit','Создал рубрику статей '.$page_title);
	...
	foreach($lang as $val){
		...
		$db=new db();
		$update=$db->update('rce_articles_cats',$data,'`pageID`=\''.$id.'\' AND `lang`=\''.$val.'\'');
	}
	rce_log($_SESSION['user']['email'],'edit','Изменил рубрику статей '.$page_title);
	header("Location: /admin/?module=".rce_get_currentdir()."&status=".$action."_ok");
	...
	$id=$_POST['id'];
	$db=new db();
	$delete=$db->delete('rce_articles_cats',"`pageID`='$id'");
	rce_log($_SESSION['user']['email'],'delete','Удалил рубрику статей с ID: '.$id);
	header("Location: /admin/?module=".rce_get_currentdir()."&status=".$action."_ok");
}
